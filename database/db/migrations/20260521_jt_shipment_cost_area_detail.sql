-- 20260521_jt_shipment_cost_area_detail.sql
-- ราย AWB สำหรับ 2 มุมมองในการ์ดวิเคราะห์การจัดส่ง:
--   (1) ต้นทุน / ปรับน้ำหนักผิดปกติ (weight anomaly)
--   (2) พื้นที่ปิดงานช้า (ตำบล/อำเภอ + สถานะปิดงาน)
--
-- weight anomaly (สูตรเดียวกับ customer-profile RPC):
--   admin_billable   = GREATEST(order_weight, width*length*total_height/6000)
--   gateway_billable = GREATEST(gateway_weight, gw_width*gw_length*gw_height/6000)
--   FLAG ⟺ admin_billable > 0 AND gateway_billable > admin_billable*2.5 AND diff > 1 kg
--
-- closed (single source of truth):
--   signer_name IS NOT NULL AND trim<>'' AND upper(trim)<>'NULL'
--
-- our_cost = billable weight × zone rate (เหมือน reconciliation/financial)
