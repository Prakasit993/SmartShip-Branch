-- Customer-profile: exclude acknowledged parcels from KPI counts.
--
-- Builds on get_customer_profile_summary v3 (20260519). Adds anti-join against
-- jt_return_acknowledgements (the generic parcel-ack table, see 20260522_jt_parcel_ack_kind):
--   overdueOver3Days / overdueOver7Days → exclude active ack kind='overdue'
--   withIssue                           → exclude active ack kind='return'
--      (issue ใช้ kind='return' ร่วมกับการ์ดตีกลับบน dashboard — ack ที่หนึ่งมีผลทั้งสองที่)
--
-- Everything else (weight, COD, financial snapshot, anomaly) is unchanged.

-- 1. allow the new 'overdue' kind on the shared ack table
alter table public.jt_return_acknowledgements
    drop constraint if exists jt_parcel_ack_kind_check;
alter table public.jt_return_acknowledgements
    add constraint jt_parcel_ack_kind_check
    check (kind in ('return', 'stagnant', 'overdue'));

-- 2. RPC v4 — same as v3 + ack exclusion on overdue/issue KPI counts
...
begin
    ...
    with matched as (
        select
            ...
            -- ✏ NEW: ack exclusion flags (active acks on the shared parcel-ack table)
            exists (
...
comment on function public.get_customer_profile_summary(text) is
    'v4 — v3 + exclude acknowledged parcels (kind=overdue from SLA counts, kind=return from withIssue)';
