-- Customer-profile RPC v5 — acknowledged parcels drop out of total/closed too.
--
-- Builds on v4 (20260522_customer_profile_ack_exclusion). Decision: รับทราบ (ซ่อน)
-- = "จัดการแล้ว ไม่นับรวม" → ตัดพัสดุที่มี active ack ออกจาก total และ closed
-- ทำให้ % สำเร็จ (closed/total) ขยับขึ้นเพราะตัวหารลด (closed ของพัสดุที่ยังไม่ปิดเท่าเดิม).
--
-- card counts ยังตัดแบบ per-kind ให้ตรงกับ drilldown:
--   overdueOver3Days / overdueOver7Days → ตัด acked_overdue (kind='overdue')
--   withIssue                           → ตัด acked_issue   (kind='return')
-- total / closed → ตัดถ้ามี ack ชนิดใดชนิดหนึ่ง (overdue หรือ return).
--
-- weight / COD / financial / anomaly = ไม่เปลี่ยน (คิดจากพัสดุทั้งหมดเหมือนเดิม).
...
begin
    ...
    with matched as (
        select
            ...
            exists (
                select 1 from public.jt_return_acknowledgements a
                where a.awb_number = s.awb_number
                  and a.status = 'active'
                  and a.kind = 'overdue'
            ) as acked_overdue,
            exists (
    ...
    kpi_agg as (
        select
            -- ✏ v5: total/closed ตัดพัสดุที่ถูก ack (kind overdue หรือ return) ออก
            count(*) filter (where not (acked_overdue or acked_issue))::bigint as total,
            count(*) filter (where is_closed and not (acked_overdue or acked_issue))::bigint as closed,
            count(*) filter (
                where not is_closed
                  and booking_d is not null
                  and (v_today - booking_d) > 3
                  and not acked_overdue
            )::bigint as overdue_3,
            count(*) filter (
                where not is_closed
                  and booking_d is not null
            ...
            count(*) filter (where has_issue and not acked_issue)::bigint as with_issue
        from matched
    ),
    weight_agg as (
        select
...
comment on function public.get_customer_profile_summary(text) is
    'v5 — acked parcels drop from total/closed (success% reflects handled); overdue/issue counts per-kind to match drilldowns';
