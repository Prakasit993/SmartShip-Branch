...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (error) {
            console.error('[tiktok-shipments/stagnant-parcels]', error);
            return NextResponse.json({ error: error.message }, { status: 500 });
        }

        // active ack (kind=stagnant) → ตัดออกจาก count/list
        const { data: ackRows, error: ackErr } = await supabaseAdmin
            ...
            .eq('status', 'active');
        if (ackErr) {
            console.warn('[tiktok-shipments/stagnant-parcels] ack fetch failed:', ackErr.message);
        }
        type AckRow = {
            awb_number: string | null;
            reason: string | null;
        ...
        console.error('[tiktok-shipments/stagnant-parcels] unexpected error:', err);
